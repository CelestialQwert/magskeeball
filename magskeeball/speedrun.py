...
class Speedrun(GameMode):
    ...
    def draw_panel(self, panel):
        ...
        panel.draw.text(
            (57, 31), "BALLS", font=res.FONTS["Medium"], fill=res.COLORS["WHITE"]
        )
        panel.draw.text(
            (66, 41),
            "%02d" % self.balls,
        ...
        panel.draw.text(
            (12, 41), "%04d" % score, font=res.FONTS["Medium"], fill=score_color
        )
        ...
        if self.time_elapsed < 0:
            ...
            panel.draw.text(
                (15, 54),
                "READY... {:1}".format(seconds),
                font=res.FONTS["Medium"],
                fill=res.COLORS["WHITE"],
            )
        elif self.time_elapsed < 2 * res.FPS:
            panel.draw.text(
                (39, 54), "GO!", font=res.FONTS["Medium"], fill=res.COLORS["WHITE"]
            )
        ...
        if self.debug:
            ...
            panel.draw.text(
                (85, 57),
                "{:02}".format(self.returned_balls),
                font=res.FONTS["Small"],
                fill=res.COLORS["ORANGE"],
            )
